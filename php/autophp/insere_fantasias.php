...
<html>
<head>
<title>Inserção de dados na tabela (maio-2020): fantasias ".$campo_busca."</title>
...
<body id='conteudo'>
<div class='cabecalio'>
<h1>Inserção de Dados na tabela (maio/2020): fantasias</h1>
...
$sql_comment="SELECT TABLE_COMMENT FROM information_schema.TABLES WHERE TABLE_NAME = 'fantasias';";
...
$sql="select count(*) from fantasias";
...
<div style='border: 1px solid black; background-color: green; width: 100%'><table style='width: 100%'><tr><td>Consulta:<input type='text' id='id_campo_de_search' data-nivel='0' placeholder='Coloque a consulta aqui!' /></td><td><input 
				type='button'
				value='id_chave_fantasia' 
				data-nivel='0' 
				name='busca_principal'
				onclick='var itz=document.getElementById(`id_campo_de_search`).value;  if (itz===``) {alert(`Você deixou o campo de busca vazio!`);} else {carrega_busca(`id_chave_fantasia`,itz,50,0);;}'
			></td><td> <input 
				type='button'
				value='nome_fantasia' 
				data-nivel='0' 
				name='busca_principal'
				onclick='var itz=document.getElementById(`id_campo_de_search`).value;  if (itz===``) {alert(`Você deixou o campo de busca vazio!`);} else {carrega_busca(`nome_fantasia`,itz,50,0);;}'
...
<tr class='cabecalio_table'><th></th><th>id</th><th>nome_fantasia</th><th>photo_filename_fantasia</th><th>time_stamp</th></tr>";
...
if ($campo_busca=="") {$sql="select * from fantasias order by nome_fantasia limit ".$limit." offset ".$offset;}
else {$sql="select * from fantasias where ".$campo_busca." like '".$valor_busca."%' order by '".$campo_busca."' limit ".$limit;}
echo $sql;
$result=$conn->query("$sql");
...
$id_chave_fantasia=$row["id_chave_fantasia"];

$id_registro=$row["id_chave_fantasia"];

$nome_fantasia=$row["nome_fantasia"];
$campos_atualizaveis=$campos_atualizaveis.",".'"campo_'.$id_registro.'_2"';
$photo_filename_fantasia=$row["photo_filename_fantasia"];
$campos_atualizaveis=$campos_atualizaveis.",".'"campo_'.$id_registro.'_3"';
$time_stamp=$row["time_stamp"];

$campos_atualizaveis=$campos_atualizaveis."]";
echo "<tr class='principal'><td>".$row_number."</td><td>".$id_chave_fantasia."</td><td>
				<input 
					...
					value='".$nome_fantasia."'
					data-alterado='nao'
					data-tabela='fantasias'
					data-campo='nome_fantasia'
                                        data-id='".$id_registro."'
        				data-nivel='0'
				/>
			   </td><td>
					<img id='campo_src_".$id_registro."_3'
					     src='".$path_imagem.str_replace("/var/www/html/","../",$photo_filename_fantasia)."' 
                                             style='background-color: white; width: 100px; height: auto'
                                             data-ja-tentei='nao' 
					     onerror='
						if (this.getAttribute(`data-ja-tentei`)==`nao`){
						var itz=`".$path_imagem.str_replace("/var/www/html/","../",$photo_filename_fantasia)."`;
					...
					<input type='button' value='Imagem do Servidor' data-nivel='0' onclick='
						...
						var w=window.open(".'""'.",".'"MostraArquivosDiretorio"'.",".'"width=1000 height=1000"'.");
                                                var resposta=".'""'.";
           					var url=".'"mostra_diretorio_imagens.php?banco=baboracex&diretorio=imagens&id_input=campo_'.$id_registro.'_3&id_input_path="'."+identificador+".'"&id_input_img=campo_src_'.$id_registro.'_3&jpg=1&png=1&gif=1&pdf=0"'.";
				...
				<input type='button' value='Amplia' data-nivel='0' 
					onclick='
						var file_=document.getElementById(".'"campo_'.$id_registro.'_3"'.").value;
						if (file_ != ".'""'.") {
				...
				'>
					...
					<input type='button' value='Sobe Imagem' data-nivel='0' onclick='
						document.getElementById(".'"campo_src_'.$id_registro.'_3"'.").setAttribute(".'"data-ja-tentei"'.",".'"nao"'."); 
							document.getElementById(".'"campo_inputfile_'.$id_registro.'_3"'.").click();'>
					<input style='display: none' data-nivel='0' id='campo_inputfile_".$id_registro."_3'
					     type='file'
                                             onchange='
                                                var resposta=".'""'.";
						var nome_arquivo=this.value.replace(/^.*[\\\/]/, ".'""'.");
						document.getElementById(".'"campo_'.$id_registro.'_3"'.").setAttribute(".'"data-alterado"'.",".'"alterado"'."); 
						document.getElementById(".'"campo_'.$id_registro.'_3"'.").value=`../imagens/`+nome_arquivo;      
						var x=document.getElementsByClassName(".'"editavel"'.");
						var i;
						for (i = 0; i < x.length; i++) {
						...
						if (this.files && this.files[0]) {
                                                        var ide=".'"campo_src_'.$id_registro.'_3"'.";
				...
				<input 
					id='campo_".$id_registro."_3' 
                                        class='editavel'
					type='text' 
					value='".$photo_filename_fantasia."'
					data-alterado='nao'
					data-tabela='fantasias'
					data-campo='photo_filename_fantasia'
                                        data-id='".$id_registro."'
        				data-nivel='0'
				/>
			   </td><td>".$time_stamp."</td>
...
mostra_botao('insercao','fantasias','0');
...
for (i=0; i<pontos.length; i++){
	...
	carrega_NtoN(pontos[i].id,'baboracex', 'fantasias','id_documento',document.getElementById(pontos[i].id).innerText);
...
ativa_alterados();
if ('fantasias'=='documentos'){percorre_ponto_insercao();}
...
mensagem_de_carregamento_ativada();
  ...
  alert('Busca pelo campo '+campo_busca);
	var resposta='';
	var url='insere_fantasias.php?offset='+offset+'&limit='+limit+'&campo_busca='+campo_busca+'&valor_busca='+valor_busca+'&scrollx='+escrolx+'&scrolly='+escroly;
        var oReq=new XMLHttpRequest();
	oReq.open('GET',url, false);
	oReq.onload= function (e) {
			    resposta=oReq.responseText;
...
function (){
	var resposta='';
	var url='insere_fantasias.php?offset=0&limit=".$limitador_registros_insere."&campo_busca=".$campo_busca."&valor_busca=".$valor_busca."&scrollx='+escrolx+'&scrolly='+escroly;
        var oReq=new XMLHttpRequest();
	oReq.open('GET',url, false);
	oReq.onload= function (e) {
			    resposta=oReq.responseText;
...
var resposta='';
var url='apaga_registro.php?banco=baboracex&tabela=fantasias&id='+id;
alert(url);
var oReq=new XMLHttpRequest();
oReq.open('GET', url, false);
oReq.onload= function (e) {
