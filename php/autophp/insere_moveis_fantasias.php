...
<html>
<head>
<title>Inserção de dados na tabela (maio-2020): moveis_fantasias ".$campo_busca."</title>
...
<body id='conteudo'>
<div class='cabecalio'>
<h1>Inserção de Dados na tabela (maio/2020): moveis_fantasias</h1>
...
$sql_comment="SELECT TABLE_COMMENT FROM information_schema.TABLES WHERE TABLE_NAME = 'moveis_fantasias';";
...
$sql="select count(*) from moveis_fantasias";
...
<div style='border: 1px solid black; background-color: green; width: 100%'><table style='width: 100%'><tr><td>Consulta:<input type='text' id='id_campo_de_search' data-nivel='0' placeholder='Coloque a consulta aqui!' /></td><td><input 
				type='button'
				value='id_chave_movel_fantasia' 
				data-nivel='0' 
				name='busca_principal'
				onclick='var itz=document.getElementById(`id_campo_de_search`).value;  if (itz===``) {alert(`Você deixou o campo de busca vazio!`);} else {carrega_busca(`id_chave_movel_fantasia`,itz,50,0);;}'
			></td><td> <input 
				type='button'
				value='nome_movel_fantasia' 
				data-nivel='0' 
				name='busca_principal'
				onclick='var itz=document.getElementById(`id_campo_de_search`).value;  if (itz===``) {alert(`Você deixou o campo de busca vazio!`);} else {carrega_busca(`nome_movel_fantasia`,itz,50,0);;}'
...
if ($campo_busca=="") {$sql="select * from moveis_fantasias order by nome_movel_fantasia limit ".$limit." offset ".$offset;}
else {$sql="select * from moveis_fantasias where ".$campo_busca." like '".$valor_busca."%' order by '".$campo_busca."' limit ".$limit;}
echo $sql;
$result=$conn->query("$sql");
...
$id_chave_movel_fantasia=$row["id_chave_movel_fantasia"];

$id_registro=$row["id_chave_movel_fantasia"];

$nome_movel_fantasia=$row["nome_movel_fantasia"];
...
echo "<tr class='principal'><td>".$row_number."</td><td>".$id_chave_movel_fantasia."</td><td>
				<input 
					...
					data-campo='nome_movel_fantasia'
                                        data-id='".$id_registro."'
        				data-nivel='0'
				/>
			   </td>
...
<div class='dropdown'>
  <input type='text' 
	...
	data-tabela='moveis_fantasias'
	data-campo='id_fantasia' 
	data-fkid='".$id_fantasia."' 
        data-default=''
	data-fk-banco='baboracex' 
	data-fk-tabela='fantasias' 
	data-fk-id='id_chave_fantasia'
	data-selecionado='-1'
	data-event-blur='NAO'
...
<div class='dropdown'>
  <input type='text' 
	...
	data-fkid='".$id_movel."' 
        data-default=''
	data-fk-banco='baboracex' 
	data-fk-tabela='moveis' 
	data-fk-id='id_chave_movel'
	data-selecionado='-1'
	data-event-blur='NAO'
	data-event-focus='NAO'
	data-event-keyup='NAO'
...
mostra_botao('insercao','moveis_fantasias','0');
...
for (i=0; i<pontos.length; i++){
	...
	carrega_NtoN(pontos[i].id,'baboracex', 'moveis_fantasias','id_documento',document.getElementById(pontos[i].id).innerText);
...
ativa_alterados();
if ('moveis_fantasias'=='documentos'){percorre_ponto_insercao();}
...
mensagem_de_carregamento_ativada();
  ...
  alert('Busca pelo campo '+campo_busca);
	var resposta='';
	var url='insere_moveis_fantasias.php?offset='+offset+'&limit='+limit+'&campo_busca='+campo_busca+'&valor_busca='+valor_busca+'&scrollx='+escrolx+'&scrolly='+escroly;
        var oReq=new XMLHttpRequest();
	oReq.open('GET',url, false);
	oReq.onload= function (e) {
			    resposta=oReq.responseText;
...
function (){
	var resposta='';
	var url='insere_moveis_fantasias.php?offset=0&limit=".$limitador_registros_insere."&campo_busca=".$campo_busca."&valor_busca=".$valor_busca."&scrollx='+escrolx+'&scrolly='+escroly;
        var oReq=new XMLHttpRequest();
	oReq.open('GET',url, false);
	oReq.onload= function (e) {
			    resposta=oReq.responseText;
...
var resposta='';
var url='apaga_registro.php?banco=baboracex&tabela=moveis_fantasias&id='+id;
alert(url);
var oReq=new XMLHttpRequest();
oReq.open('GET', url, false);
oReq.onload= function (e) {
