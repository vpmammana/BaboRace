...
<html>
<head>
<title>Inserção de dados na tabela (maio-2020): operacoes_on_line ".$campo_busca."</title>
...
<body id='conteudo'>
<div class='cabecalio'>
<h1>Inserção de Dados na tabela (maio/2020): operacoes_on_line</h1>
...
$sql_comment="SELECT TABLE_COMMENT FROM information_schema.TABLES WHERE TABLE_NAME = 'operacoes_on_line';";
...
$sql="select count(*) from operacoes_on_line";
...
<div style='border: 1px solid black; background-color: green; width: 100%'><table style='width: 100%'><tr><td>Consulta:<input type='text' id='id_campo_de_search' data-nivel='0' placeholder='Coloque a consulta aqui!' /></td><td><input 
				type='button'
				value='id_chave_operacao_on_line' 
				data-nivel='0' 
				name='busca_principal'
				onclick='var itz=document.getElementById(`id_campo_de_search`).value;  if (itz===``) {alert(`Você deixou o campo de busca vazio!`);} else {carrega_busca(`id_chave_operacao_on_line`,itz,50,0);;}'
			></td><td> <input 
				type='button'
				value='nome_time_stamp' 
				data-nivel='0' 
				name='busca_principal'
				onclick='var itz=document.getElementById(`id_campo_de_search`).value;  if (itz===``) {alert(`Você deixou o campo de busca vazio!`);} else {carrega_busca(`nome_time_stamp`,itz,50,0);;}'
...
<tr class='cabecalio_table'><th></th><th>id</th><th>nome_time_stamp</th><th>id_registrado</th><th>id_operacao</th></tr>";
...
if ($campo_busca=="") {$sql="select * from operacoes_on_line order by nome_time_stamp limit ".$limit." offset ".$offset;}
else {$sql="select * from operacoes_on_line where ".$campo_busca." like '".$valor_busca."%' order by '".$campo_busca."' limit ".$limit;}
echo $sql;
$result=$conn->query("$sql");
...
$id_chave_operacao_on_line=$row["id_chave_operacao_on_line"];

$id_registro=$row["id_chave_operacao_on_line"];

$nome_time_stamp=$row["nome_time_stamp"];

$id_registrado=$row["id_registrado"];

$id_operacao=$row["id_operacao"];

$campos_atualizaveis=$campos_atualizaveis."]";
echo "<tr class='principal'><td>".$row_number."</td><td>".$id_chave_operacao_on_line."</td><td>".$nome_time_stamp."</td>

<td>
<div class='dropdown'>
  <input type='text' 
	id='drop_".$id_registro."_3' 
	class='dropbtn' 
	onfocusout='document.getElementById(".'"'."lista_".$id_registro."_3".'"'.").setAttribute(".'"'."data-keyup".'"'.",".'"'."inativo".'"'.");document.getElementById(".'"'."drop_".$id_registro."_3".'"'.").setAttribute(".'"'."data-selecionado".'"'.",".'"'."-1".'"'."); document.getElementById(".'"'."drop_".$id_registro."_3".'"'.").setAttribute(".'"'."data-n-itens".'"'.",".'"'."0".'"'.");' 
        data-drop='lista_".$id_registro."_3'
        data-momento='atualizacao'
	data-id='".$id_registro."'
        data-max-itens='100'
	data-banco='baboracex' 
	data-tabela='operacoes_on_line'
	data-campo='id_registrado' 
	data-fkid='".$id_registrado."' 
        data-default=''
	data-fk-banco='baboracex' 
	data-fk-tabela='registrados' 
	data-fk-id='id_chave_registrado'
	data-selecionado='-1'
	data-event-blur='NAO'
  ...
  <div id='lista_".$id_registro."_3' class='dropdown-content'  data-keyup='inativo'>
  </div>
</div>
</td>
...
<div class='dropdown'>
  <input type='text' 
	id='drop_".$id_registro."_4' 
	class='dropbtn' 
	onfocusout='document.getElementById(".'"'."lista_".$id_registro."_4".'"'.").setAttribute(".'"'."data-keyup".'"'.",".'"'."inativo".'"'.");document.getElementById(".'"'."drop_".$id_registro."_4".'"'.").setAttribute(".'"'."data-selecionado".'"'.",".'"'."-1".'"'."); document.getElementById(".'"'."drop_".$id_registro."_4".'"'.").setAttribute(".'"'."data-n-itens".'"'.",".'"'."0".'"'.");' 
        data-drop='lista_".$id_registro."_4'
        data-momento='atualizacao'
	data-id='".$id_registro."'
        data-max-itens='100'
	data-banco='baboracex' 
	data-tabela='operacoes_on_line'
	data-campo='id_operacao' 
	data-fkid='".$id_operacao."' 
        data-default=''
	data-fk-banco='baboracex' 
	data-fk-tabela='tipos_operacoes_on_line' 
	data-fk-id='id_chave_tipo_operacao_on_line'
	data-selecionado='-1'
	data-event-blur='NAO'
	data-event-focus='NAO'
	data-event-keyup='NAO'
  ...
  <div id='lista_".$id_registro."_4' class='dropdown-content'  data-keyup='inativo'>
  </div>
</div>
</td>
...
mostra_botao('insercao','operacoes_on_line','0');
...
for (i=0; i<pontos.length; i++){
	...
	carrega_NtoN(pontos[i].id,'baboracex', 'operacoes_on_line','id_documento',document.getElementById(pontos[i].id).innerText);
...
ativa_alterados();
if ('operacoes_on_line'=='documentos'){percorre_ponto_insercao();}
...
function carrega_drop_btn(element){
  ...
  if(element.getAttribute('data-momento')=='atualizacao'){
           var resposta='';
           var url='auto_ler_tabela_campo.php?banco=baboracex&tabela='+element.getAttribute('data-fk-tabela')+'&campo_id='+element.getAttribute('data-fk-id')+'&id='+element.getAttribute('data-fkid');
           var oReq=new XMLHttpRequest();
           oReq.open('GET', url, false);
           oReq.onload = function (e) {
                     resposta=oReq.responseText;
...
// se o dropbtn for de inserao de dados, ao inves de atualização, nao faz sentido buscar dados na base, porque o campo tem que estar vazio
  if(x[i].getAttribute('data-momento')=='atualizacao'){
           var resposta='';
           var url='auto_ler_tabela_campo.php?banco=baboracex&tabela='+x[i].getAttribute('data-fk-tabela')+'&campo_id='+x[i].getAttribute('data-fk-id')+'&id='+x[i].getAttribute('data-fkid');
           var oReq=new XMLHttpRequest();
           oReq.open('GET', url, false);
           oReq.onload = function (e) {
                     resposta=oReq.responseText;
...
mensagem_de_carregamento_ativada();
  ...
  alert('Busca pelo campo '+campo_busca);
	var resposta='';
	var url='insere_operacoes_on_line.php?offset='+offset+'&limit='+limit+'&campo_busca='+campo_busca+'&valor_busca='+valor_busca+'&scrollx='+escrolx+'&scrolly='+escroly;
        var oReq=new XMLHttpRequest();
	oReq.open('GET',url, false);
	oReq.onload= function (e) {
			    resposta=oReq.responseText;
...
function (){
	var resposta='';
	var url='insere_operacoes_on_line.php?offset=0&limit=".$limitador_registros_insere."&campo_busca=".$campo_busca."&valor_busca=".$valor_busca."&scrollx='+escrolx+'&scrolly='+escroly;
        var oReq=new XMLHttpRequest();
	oReq.open('GET',url, false);
	oReq.onload= function (e) {
			    resposta=oReq.responseText;
...
var resposta='';
var url='apaga_registro.php?banco=baboracex&tabela='+tabela+'&id='+id;
alert(url);
var oReq=new XMLHttpRequest();
oReq.open('GET', url, false);
oReq.onload= function (e) {
...
var resposta='';
var url='apaga_registro.php?banco=baboracex&tabela=operacoes_on_line&id='+id;
alert(url);
var oReq=new XMLHttpRequest();
oReq.open('GET', url, false);
oReq.onload= function (e) {
...
var resposta='';
var url='insere_registro.php?banco=baboracex&tabela='+tabela+'&campos='+acumula_campos+'&valores='+acumula_valores;
alert(url);
var oReq=new XMLHttpRequest();
oReq.open('GET', url, false);
oReq.onload= function (e) {
...
function painel_insercao(div_insercao, tabela){
	   nivel_insercao++;
           var resposta='';
           var url='insercao.php?banco=baboracex&tabela='+tabela+'&nivel='+nivel_insercao;
           var oReq=new XMLHttpRequest();
           oReq.open('GET', url, false);
           oReq.onload = function (e) {
                     resposta=oReq.responseText;
...
function atualiza_fk (id_elemento_campo){
		...
		var campo=document.getElementById(id_elemento_campo);
	                var resposta='';
	                var url='atualiza_campos.php?banco=baboracex&tabela='+campo.getAttribute('data-tabela')+'&campo='+campo.getAttribute('data-campo')+'&id='+campo.getAttribute('data-id')+'&valor='+campo.getAttribute('data-fkid');;
	                var oReq=new XMLHttpRequest();
			oReq.open('GET', url, false);
			oReq.onload = function (e) {
				resposta=oReq.responseText;
	...
	function minhafuncao(item, index){
		var campo=document.getElementById(item);
                if (campo.getAttribute('data-alterado')=='alterado'){
	                var resposta='';
	                var url='atualiza_campos.php?banco=baboracex&tabela='+campo.getAttribute('data-tabela')+'&campo='+campo.getAttribute('data-campo')+'&id='+campo.getAttribute('data-id')+'&valor='+campo.value;
	                var oReq=new XMLHttpRequest();
			oReq.open('GET', url, false);
			oReq.onload = function (e) {
				resposta=oReq.responseText;
