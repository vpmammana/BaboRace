...
<html>
<head>
<title>Inserção de dados na tabela (maio-2020): registrados ".$campo_busca."</title>
...
<body id='conteudo'>
<div class='cabecalio'>
<h1>Inserção de Dados na tabela (maio/2020): registrados</h1>
...
$sql_comment="SELECT TABLE_COMMENT FROM information_schema.TABLES WHERE TABLE_NAME = 'registrados';";
...
$sql="select count(*) from registrados";
...
<div style='border: 1px solid black; background-color: green; width: 100%'><table style='width: 100%'><tr><td>Consulta:<input type='text' id='id_campo_de_search' data-nivel='0' placeholder='Coloque a consulta aqui!' /></td><td><input 
				type='button'
				value='id_chave_registrado' 
				data-nivel='0' 
				name='busca_principal'
				onclick='var itz=document.getElementById(`id_campo_de_search`).value;  if (itz===``) {alert(`Você deixou o campo de busca vazio!`);} else {carrega_busca(`id_chave_registrado`,itz,50,0);;}'
			></td><td> <input 
				type='button'
				value='nome_registrado' 
				data-nivel='0' 
				name='busca_principal'
				onclick='var itz=document.getElementById(`id_campo_de_search`).value;  if (itz===``) {alert(`Você deixou o campo de busca vazio!`);} else {carrega_busca(`nome_registrado`,itz,50,0);;}'
...
if ($campo_busca=="") {$sql="select * from registrados order by nome_registrado limit ".$limit." offset ".$offset;}
else {$sql="select * from registrados where ".$campo_busca." like '".$valor_busca."%' order by '".$campo_busca."' limit ".$limit;}
echo $sql;
$result=$conn->query("$sql");
...
$id_chave_registrado=$row["id_chave_registrado"];

$id_registro=$row["id_chave_registrado"];
...
echo "<tr class='principal'><td>".$row_number."</td><td>".$id_chave_registrado."</td><td>
				<input 
...
<div class='dropdown'>
  <input type='text' 
	...
	data-tabela='registrados'
	data-campo='id_on_line' 
	data-fkid='".$id_on_line."' 
        data-default=''
...
<div class='dropdown'>
  <input type='text' 
	id='drop_".$id_registro."_5' 
	class='dropbtn' 
	onfocusout='document.getElementById(".'"'."lista_".$id_registro."_5".'"'.").setAttribute(".'"'."data-keyup".'"'.",".'"'."inativo".'"'.");document.getElementById(".'"'."drop_".$id_registro."_5".'"'.").setAttribute(".'"'."data-selecionado".'"'.",".'"'."-1".'"'."); document.getElementById(".'"'."drop_".$id_registro."_5".'"'.").setAttribute(".'"'."data-n-itens".'"'.",".'"'."0".'"'.");' 
        data-drop='lista_".$id_registro."_5'
        data-momento='atualizacao'
	data-id='".$id_registro."'
        data-max-itens='100'
	data-banco='baboracex' 
  ...
  <div id='lista_".$id_registro."_5' class='dropdown-content'  data-keyup='inativo'>
  </div>
</div>
</td>
<td>".$time_stamp."</td>
...
mostra_botao('insercao','registrados','0');
...
for (i=0; i<pontos.length; i++){
	...
	carrega_NtoN(pontos[i].id,'baboracex', 'registrados','id_documento',document.getElementById(pontos[i].id).innerText);
...
ativa_alterados();
if ('registrados'=='documentos'){percorre_ponto_insercao();}
...
mensagem_de_carregamento_ativada();
  ...
  alert('Busca pelo campo '+campo_busca);
	var resposta='';
	var url='insere_registrados.php?offset='+offset+'&limit='+limit+'&campo_busca='+campo_busca+'&valor_busca='+valor_busca+'&scrollx='+escrolx+'&scrolly='+escroly;
        var oReq=new XMLHttpRequest();
	oReq.open('GET',url, false);
	oReq.onload= function (e) {
			    resposta=oReq.responseText;
...
function (){
	var resposta='';
	var url='insere_registrados.php?offset=0&limit=".$limitador_registros_insere."&campo_busca=".$campo_busca."&valor_busca=".$valor_busca."&scrollx='+escrolx+'&scrolly='+escroly;
        var oReq=new XMLHttpRequest();
	oReq.open('GET',url, false);
	oReq.onload= function (e) {
			    resposta=oReq.responseText;
...
var resposta='';
var url='apaga_registro.php?banco=baboracex&tabela=registrados&id='+id;
alert(url);
var oReq=new XMLHttpRequest();
oReq.open('GET', url, false);
oReq.onload= function (e) {
