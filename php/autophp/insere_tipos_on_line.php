...
<html>
<head>
<title>Inserção de dados na tabela (maio-2020): tipos_on_line ".$campo_busca."</title>
...
<body id='conteudo'>
<div class='cabecalio'>
<h1>Inserção de Dados na tabela (maio/2020): tipos_on_line</h1>
...
$sql_comment="SELECT TABLE_COMMENT FROM information_schema.TABLES WHERE TABLE_NAME = 'tipos_on_line';";
...
$sql="select count(*) from tipos_on_line";
...
<div style='border: 1px solid black; background-color: green; width: 100%'><table style='width: 100%'><tr><td>Consulta:<input type='text' id='id_campo_de_search' data-nivel='0' placeholder='Coloque a consulta aqui!' /></td><td><input 
				type='button'
				value='id_chave_tipo_on_line' 
				data-nivel='0' 
				name='busca_principal'
				onclick='var itz=document.getElementById(`id_campo_de_search`).value;  if (itz===``) {alert(`Você deixou o campo de busca vazio!`);} else {carrega_busca(`id_chave_tipo_on_line`,itz,50,0);;}'
			></td><td> <input 
				type='button'
				value='nome_on_line' 
				data-nivel='0' 
...
if ($campo_busca=="") {$sql="select * from tipos_on_line order by nome_on_line limit ".$limit." offset ".$offset;}
else {$sql="select * from tipos_on_line where ".$campo_busca." like '".$valor_busca."%' order by '".$campo_busca."' limit ".$limit;}
echo $sql;
$result=$conn->query("$sql");
...
$id_chave_tipo_on_line=$row["id_chave_tipo_on_line"];

$id_registro=$row["id_chave_tipo_on_line"];
...
echo "<tr class='principal'><td>".$row_number."</td><td>".$id_chave_tipo_on_line."</td><td>
				<input 
...
mostra_botao('insercao','tipos_on_line','0');
...
for (i=0; i<pontos.length; i++){
	...
	carrega_NtoN(pontos[i].id,'baboracex', 'tipos_on_line','id_documento',document.getElementById(pontos[i].id).innerText);
...
ativa_alterados();
if ('tipos_on_line'=='documentos'){percorre_ponto_insercao();}
...
mensagem_de_carregamento_ativada();
  ...
  alert('Busca pelo campo '+campo_busca);
	var resposta='';
	var url='insere_tipos_on_line.php?offset='+offset+'&limit='+limit+'&campo_busca='+campo_busca+'&valor_busca='+valor_busca+'&scrollx='+escrolx+'&scrolly='+escroly;
        var oReq=new XMLHttpRequest();
	oReq.open('GET',url, false);
	oReq.onload= function (e) {
			    resposta=oReq.responseText;
...
function (){
	var resposta='';
	var url='insere_tipos_on_line.php?offset=0&limit=".$limitador_registros_insere."&campo_busca=".$campo_busca."&valor_busca=".$valor_busca."&scrollx='+escrolx+'&scrolly='+escroly;
        var oReq=new XMLHttpRequest();
	oReq.open('GET',url, false);
	oReq.onload= function (e) {
			    resposta=oReq.responseText;
...
var resposta='';
var url='apaga_registro.php?banco=baboracex&tabela=tipos_on_line&id='+id;
alert(url);
var oReq=new XMLHttpRequest();
oReq.open('GET', url, false);
oReq.onload= function (e) {
